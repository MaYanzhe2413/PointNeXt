...
def test_basic_functionality():
    ...
    # BlockWiseTransfer已经是nn.Module，会自动处理设备
...
def visualize_results():
    ...
    try:
        ...
        # 原始数据
        ax1 = fig.add_subplot(141, projection='3d')
        ax1.scatter(xA_np[:, 0], xA_np[:, 1], xA_np[:, 2], c='blue', alpha=0.6, s=20, label=f'Frame A ({xA.shape[0]})')
        ax1.scatter(xB_np[:, 0], xB_np[:, 1], xB_np[:, 2], c='red', alpha=0.6, s=20, label=f'Frame B ({xB.shape[0]})')
