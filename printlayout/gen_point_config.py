#!/usr/bin/env python3
"""
Unified PNN point_config generator (single file)

- Instantiates the model from a YAML cfg and observes real shapes via a dummy forward pass
- Optionally infers N0 (input points) from cfg or from a real DataLoader batch
- Outputs a point_config.py-style OrderedDict suitable for the simulator

Usage examples:
  python tools/gen_point_config.py --cfg PointNeXt/cfgs/modelnet40ply2048/pointnet++.yaml \
  --num-points 1024 --device cuda --out simulator/PNN_Acc_Base/generated/pointnetpp_from_model.py

  # Auto-detect N0 from cfg
  python tools/gen_point_config.py --cfg <full-train-yaml> --auto-num-points --device cuda --out <out>

  # Infer N0 from DataLoader (requires valid dataset path)
  python tools/gen_point_config.py --cfg <full-train-yaml> --infer-from-data --device cuda --out <out>
"""
import argparse
import os
import sys
...
def _infer_num_points_from_args(dataset_name: str, data_dir: str, split: str = 'val', dataset_num_points: Optional[int] = None) -> Optional[int]:
  try:
    # Minimal transforms: PointsToTensor to ensure torch tensors
    datatransforms_cfg = edict({
      split: ['PointsToTensor']
    })
    # Build minimal dataset cfg depending on dataset
    if dataset_name.lower() == 's3dis':
      common = edict({'NAME': 'S3DIS', 'data_root': data_dir, 'test_area': 5, 'voxel_size': 0.04})
      split_cfg = edict({'split': split, 'voxel_max': 24000, 'presample': False})
    else:
      common = edict({'NAME': dataset_name, 'data_dir': data_dir})
      split_cfg = edict({'split': split})
      if dataset_num_points is not None:
        split_cfg.num_points = int(dataset_num_points)

    dataset_cfg = edict({'common': common, split: split_cfg})
    # For non-S3DIS datasets like ModelNet, ensure num_points is set if provided
    if dataset_name.lower() != 's3dis' and dataset_num_points is not None:
      setattr(dataset_cfg[split], 'num_points', int(dataset_num_points))
    dataloader_cfg = edict({'num_workers': 0})
    loader = build_dataloader_from_cfg(
      batch_size=1,
      dataset_cfg=dataset_cfg,
      dataloader_cfg=dataloader_cfg,
...
def generate_from_model(cfg_path: str, num_points: int, th: int = 64, device: str = 'cpu', auto_num_points: bool = False, infer_from_data: bool = False,
                        dataset_name: Optional[str] = None, data_dir: Optional[str] = None, split: str = 'val', dataset_num_points: Optional[int] = None) -> str:
  cfg = EasyConfig()
  ...
  if inferred_n is None and infer_from_data:
    # Prefer explicit dataset args if provided
    if dataset_name and data_dir:
      inferred_n = _infer_num_points_from_args(dataset_name, data_dir, split=split, dataset_num_points=dataset_num_points)
  ...
  for i, st in enumerate(stages):
    ...
    radius = st.radius
    # Fallback to cfg-provided list if indexable
    if radius is None:
      try:
    ...
    if is_group_all:
      ...
      lines.append(
        f"        ('grouper{i+1}', [{N_in}, {N_out}, batch_size, {N_in}, None]),  # GroupAll"
      )
      group_w = N_in
    else:
      group_w = K
...
def main():
  ...
  # Optional dataset overrides (avoid providing a dataset YAML):
  ap.add_argument('--dataset-name', type=str, help='Dataset class name registered in openpoints (e.g., ModelNet40Ply2048, S3DIS)')
  ap.add_argument('--data-dir', type=str, help='Dataset directory path')
  ap.add_argument('--split', type=str, default='val', choices=['train','val','test'], help='Split to sample from when inferring from data')
  ap.add_argument('--dataset-num-points', type=int, help='Optional per-sample point count for dataset (e.g., 1024 for ModelNet)')
  args = ap.parse_args()

  content = generate_from_model(
    args.cfg,
    args.num_points,
    th=args.th,
    device=args.device,
    auto_num_points=args.auto_num_points,
    infer_from_data=args.infer_from_data,
    dataset_name=args.dataset_name,
    data_dir=args.data_dir,
    split=args.split,
    dataset_num_points=args.dataset_num_points,
  )
